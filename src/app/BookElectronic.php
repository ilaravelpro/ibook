...
class BookElectronic extends \iLaravel\Core\iApp\Model
{
    ...
    {
        return $this->belongsToMany(imodal('Book'), 'books_electronics')->withPivot(['link']);
    }
        ...
        switch ($action) {
            ...
            case 'update':
                $rules = array_merge($rules, [
                    ...
                    'status' => 'nullable|in:' . join( ',', iconfig('status.book_electronics', iconfig('status.global'))),
                ]);
                break;
            case 'additional':
